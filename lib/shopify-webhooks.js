...
export async function claimShopifyWebhookEvent({admin,eventId,eventType,payloadHash,now=new Date(),staleAfterMs=SHOPIFY_WEBHOOK_CLAIM_STALE_MS}={}){
  ...
  if(!insertError)return {claimed:true,reclaimed:false,attempts:1,claimedAt};
  ...
  return {claimed:true,reclaimed:true,attempts:reclaimed.attempts||attempts,claimedAt,reason:decision.reason};
}

export async function finalizeShopifyWebhookEvent({admin,eventId,claim,status,failureReason=null,now=new Date()}={}){
  if(!admin||!eventId||!claim?.claimed||!claim?.claimedAt||!Number.isFinite(Number(claim?.attempts)))throw new Error('Webhook finalization requires the active claim token.');
  if(!['completed','failed'].includes(status))throw new Error('Webhook finalization status is invalid.');
  const at=now instanceof Date?now:new Date(now);
  if(Number.isNaN(at.getTime()))throw new Error('Webhook finalization time is invalid.');
  const patch={status,processed_at:at.toISOString(),failure_reason:status==='failed'?String(failureReason||'').slice(0,500):null};
  const {data,error}=await admin.from('integration_webhook_events').update(patch)
    .eq('provider','shopify').eq('event_id',String(eventId)).eq('status','processing')
    .eq('attempts',Number(claim.attempts)).eq('claimed_at',String(claim.claimedAt))
    .select('status,attempts').maybeSingle();
  if(error)return {updated:false,error};
  if(!data)return {updated:false,stale:true};
  return {updated:true,status:data.status||status,attempts:data.attempts||Number(claim.attempts)};
}
