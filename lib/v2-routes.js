...
import {verifyShopifyWebhookHmac,ensureShopifyWebhooks,claimShopifyWebhookEvent,finalizeShopifyWebhookEvent} from './shopify-webhooks.js';
...
export function registerV2PreJsonRoutes(app,{admin,appUrl,env=process.env}={}){
  app.post('/api/integrations/shopify/webhook',express.raw({type:'application/json',limit:'512kb'}),async(req,res)=>{
    ...
    const claim=await claimShopifyWebhookEvent({admin,eventId,eventType:topic,payloadHash});
    if(claim.error){
      if(isStorageMissing(claim.error))return res.status(503).send('V2 storage is not ready.');
      console.error('shopify-webhook-claim',claim.error);return res.status(500).send('Webhook could not be claimed.');
    }
    if(claim.rejected)return res.status(409).send('Webhook id was reused with a different payload.');
    if(!claim.claimed)return res.json({received:true,duplicate:true,status:claim.status||'processing'});
    try{
      ...
      const finalized=await finalizeShopifyWebhookEvent({admin,eventId,claim,status:'completed'});
      if(finalized.error){console.error('shopify-webhook-finalize',finalized.error);return res.status(500).send('Webhook finalization failed.');}
      if(finalized.stale)return res.json({received:true,superseded:true});
      return res.json({received:true,reclaimed:Boolean(claim.reclaimed)});
    }catch(error){
      console.error('shopify-webhook',error);
      const finalized=await finalizeShopifyWebhookEvent({admin,eventId,claim,status:'failed',failureReason:safeError(error)}).catch(finalizeError=>({updated:false,error:finalizeError}));
      if(finalized.error)console.error('shopify-webhook-failure-finalize',finalized.error);
      if(finalized.stale)return res.json({received:true,superseded:true});
...
export function registerV2Routes(app,{admin,appUrl,requireProUser,env=process.env}={}){
  ...
  app.get('/api/v2/integrations',async(req,res)=>{
    ...
    const {data,error}=await admin.from('integration_connections').select('id,brand_id,provider,external_account_id,external_account_name,status,scopes,access_token_expires_at,refresh_token_expires_at,last_synced_at,created_at,updated_at').eq('owner_id',gate.user.id).order('created_at');
  ...
  app.post('/api/v2/operating-analysis',async(req,res)=>{
    ...
    const cashGates=reorders.map(item=>({sku:item.sku,gate:reorderCashGate(item.result,{forecastHeadroom:forecast?.minimumHeadroom??null,depositPct:req.body?.depositPct??100})}));
