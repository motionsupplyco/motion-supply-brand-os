...
async function fetchWithTimeout(url,{fetchImpl=fetch,timeoutMs=5000,headers={},maxRedirects=3}={}){
  const controller=new AbortController();
  const timer=setTimeout(()=>controller.abort(),timeoutMs);
  try{
    let current=new URL(url);
    for(let redirects=0;redirects<=maxRedirects;redirects++){
      const response=await fetchImpl(current.toString(),{headers,signal:controller.signal,redirect:'manual'});
      if(!REDIRECT_STATUSES.has(response.status))return response;
      const location=response.headers?.get?.('location');
      if(!location)throw Object.assign(new Error('RDAP redirect response did not include a location.'),{code:'RDAP_REDIRECT_INVALID'});
      if(redirects===maxRedirects)throw Object.assign(new Error('RDAP redirect limit exceeded.'),{code:'RDAP_REDIRECT_LIMIT'});
      const next=new URL(location,current);
      if(next.protocol!=='https:'||next.origin!==current.origin)throw Object.assign(new Error('RDAP redirect was blocked because it changed origin or protocol.'),{code:'RDAP_REDIRECT_BLOCKED'});
      current=next;
    }
    throw Object.assign(new Error('RDAP redirect limit exceeded.'),{code:'RDAP_REDIRECT_LIMIT'});
...
export async function lookupDomainRdap(input,{fetchImpl=fetch,timeoutMs=5000,bootstrap=null}={}){
  ...
  try{response=await fetchWithTimeout(url,{fetchImpl,timeoutMs,headers:{Accept:'application/rdap+json, application/json','User-Agent':'Motion-Supply-Brand-OS/1.0'}})}
    ...
    const reason=error?.name==='AbortError'?'RDAP lookup timed out.':error?.code==='RDAP_REDIRECT_BLOCKED'?'RDAP redirect was blocked because it left the authoritative HTTPS origin.':'RDAP lookup failed.';
    return {domain,status:'unknown',registered:null,source:'rdap',reason,checkedAt:new Date().toISOString()}
  }
