...
create table if not exists public.business_memory (
  ...
  unique(owner_id, brand_id, memory_key),
...
-- Deletion queue and Stripe event ledger are server-only.
revoke all on public.account_deletion_requests, public.stripe_webhook_events from anon, authenticated;

-- Business Memory is a Pro feature. Server service role still bypasses RLS.
