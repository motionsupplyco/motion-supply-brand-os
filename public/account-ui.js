  ...
  await withPending(button,kind==='signin'?'Signing in…':'Creating account…',async()=>{
    ...
    try{
      ...
      if(data.confirmationRequired){
        modal(`<div class="authscreen"><span class="kicker">ONE MORE STEP</span><h2>Check your email</h2><p>We sent a confirmation link to <strong>${escapeHtml(email)}</strong>.</p><p class="mini">Open that link to finish creating your account. If it doesn’t arrive in a few minutes, check Spam or Promotions.</p><div class="split mt"><button id="acctAfterSignupSignin" class="outline" type="button">Sign in</button><button id="acctAfterSignupRecover" class="outline" type="button">Forgot password</button></div><p class="mini mt">Still no email? If you may already have an account, use Sign in or Forgot password.</p></div>`);
...
async function showForgotPassword(){
  const config=await publicConfig();
  modal(`<div class="authscreen"><span class="kicker">ACCOUNT RECOVERY</span><h2>Reset your password</h2><p class="mini">Enter the email tied to your Brand OS account. We’ll send a secure recovery link if the account exists.</p><div class="authform"><label class="mini" for="acctEmail">Email</label><input id="acctEmail" type="email" autocomplete="email" inputmode="email" autocapitalize="none" spellcheck="false" placeholder="you@example.com"><button id="acctRecover" class="primary">Send recovery link</button><div id="acctMsg" class="mini" role="status" aria-live="polite"></div><button id="acctBackSignin" class="outline" type="button">Back to sign in</button>${legalLinks(config)}</div></div>`);
