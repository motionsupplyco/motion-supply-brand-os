...
test('SKU planning API is Pro-gated and owner/brand scoped',()=>{
  for(const route of ["app.get('/api/v2/sku-planning'","app.post('/api/v2/sku-planning'","app.delete('/api/v2/sku-planning/:id'"]){
    ...
    const block=routes.slice(start,start+2600);
...
test('planning rows keep supplier assumptions separate from synced inventory',()=>{
  for(const field of ['lead_weeks','high_weekly_demand','moq','deposit_pct','landed_cost','supplier_notes'])assert.match(routes,new RegExp(field));
